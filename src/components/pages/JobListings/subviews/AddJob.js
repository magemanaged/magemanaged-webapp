...
function AddJob(props) {
  ...
  function SubmitButton(){
    if (positionName && startDate && location && !pendingSubmit){
      return <Button type="submit" valid={true} form="new_job_position_form" buttonSize={'btn--medium'} buttonStyle={'btn--submit'}>Submit</Button>
    } else {
      return <Button type="submit" valid={false} form="new_job_position_form" buttonSize={'btn--medium'} buttonStyle={'btn--submit'}>Submit</Button>
    };
  };
  ...
  return (
    <div className={`add-job-view ${props.selected === props.name ? 'show' : 'hide'}`}>
      <div className="add-job-card page-view-card">
        <div className="job-add-title">
          Add a new job
        </div>
        <div className="job-sub-title">
        Adding job positions helps to ensure that all licenses and assets are available by the start date.
        </div>
          ...
          <form id='new_job_position_form' onSubmit={submit}>
            <div className="input-area">
              <div className='input-flex'>
                ...
                <input id='job_title' className='input-type'  name='job_title' type="text" placeholder={"Enter job position here."} onChange={(e) => setPosition(e.target.value)} required/>
              </div>
              <div className='input-flex'>
                <label htmlFor="start_date">Estimated start date</label>
                <input id='estimated_start_date' className='input-type' name='start_date' type="date" placeholder={"Enter estimated start date here."} onChange={(e) => setDate(e.target.value)} required/>
              </div>
              <div className='input-flex'>
                <label htmlFor="city_location">Position Location</label>
                <input id='city_location' className='input-type' name='city_location' type="text" placeholder={"Enter city here."} onChange={(e) => setLocation(e.target.value)} required/>
              </div>
            </div>
            <SubmitButton />
          </form>
